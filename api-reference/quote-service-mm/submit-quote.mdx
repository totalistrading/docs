...
<ParamField body="payout_odds" type="number" required>
  Payout multiplier on the **net stake** (`user_stake`), not the gross `bet_amount`. Between 1.0001 and 1000. The server derives all cost fields from this value and the net stake: `user_cost = bet_amount - taker_fee` (= `user_stake`), `total_payout = user_cost * payout_odds`, `mm_cost = total_payout - user_cost`.
</ParamField>
  ...
  <Expandable title="data">
    <ResponseField name="quote" type="object">
      ...
      <Expandable title="quote">
        ...
        <ResponseField name="user_cost" type="number">The net stake in USDC (`bet_amount - taker_fee`; equals the request's `user_stake`).</ResponseField>
        <ResponseField name="total_payout" type="number">Total payout in USDC (server-derived: `user_cost * payout_odds`).</ResponseField>
    ...
    <ResponseField name="taker_fee_bps" type="integer">The taker fee rate this quote was priced at, in basis points (`100` is 1%). Echoed so you can reconcile the returned `user_cost` against the base you priced, and tell a fee-on quote from a fee-off one.</ResponseField>
  </Expandable>
</ResponseField>
