...
When the user edits the legs or bet amount, the request's `version` and `request_hash` change. Listen for `quote_request:updated` on the [SSE stream](/api-reference/quote-service-mm/stream) to detect the change and re-price.
...
<ParamField body="payout_odds" type="number" required>
  Payout multiplier on the net stake (`user_stake`), never the gross `bet_amount`; see [Fees](/guides/limits-and-fees#fees). Between 1.0001 and 1000. The server derives `user_cost`, `total_payout`, and `mm_cost` from it.
</ParamField>
  ...
  <Expandable title="data">
    <ResponseField name="quote" type="object">
      ...
      <Expandable title="quote">
        ...
        <ResponseField name="total_payout" type="number">Total payout in USDC (`user_cost * payout_odds`).</ResponseField>
    ...
    <ResponseField name="taker_fee_bps" type="integer">The taker fee rate this quote was priced at, in basis points (`100` is 1%). Use it to reconcile `user_cost` against the base you priced, and to tell a fee-on quote from a fee-off one.</ResponseField>
  </Expandable>
</ResponseField>
