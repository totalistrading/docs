...
<ParamField body="payout_odds" type="number" required>
  Payout multiplier on the net stake (`user_stake`), never the gross `bet_amount`; see [Fees](/guides/limits-and-fees#fees). At least 1.0001. The server derives `user_cost`, `total_payout`, and `mm_cost` from it.
</ParamField>
