...
es.addEventListener('quote_request', (e) => {
  ...
  console.log('New request:', req.id, req.user_stake ?? req.bet_amount, 'USDC');  // price against net user_stake; falls back to gross bet_amount if absent
...
es.addEventListener('quote_request_expired', (e) => {
  const { id, reason, won } = JSON.parse(e.data);
...
es.addEventListener('cashout_request', (e) => {
  const cr = JSON.parse(e.data);
  // Distinct from quote_request: the key is cr.cashout_request_id (NOT cr.id).
  ...
  console.log('Buyback request:', cr.cashout_request_id, 'position', cr.position_id);
});
```
