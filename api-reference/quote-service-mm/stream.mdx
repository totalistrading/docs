...
| `event_ticker` | The Kalshi event ticker, e.g. `KXBTC-26JUN01`. | The Gamma **`event_slug`**, e.g. `will-bitcoin-hit-100k-in-june`. A slug, not a ticker. Always populated, but if Gamma gave us no event slug it falls back to the market slug, then to the `condition_id` — so treat it as a label, not a key. |
| `side` | `"yes"` or `"no"` | Same. |
...
Anything we can't classify — including a value your client doesn't recognise from a future release — arrives as `unresolved`, never as an unknown string. `resolution` is not the same as `side` (always `yes`/`no`).
