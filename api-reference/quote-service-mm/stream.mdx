...
This stream sends seven event names. Their spelling isn't consistent: `quote_request` and `quote_request_expired` are underscore-separated, while `quote_request:updated` is colon-separated. The names are fixed, so match them exactly as listed:
...
`mm_id` is your Privy DID, the same value as the `user_id` returned by `auth:success` on the [WebSocket](/guides/websocket#authentication), and the key of your `mm:quotes:{mm_id}` channel there.
...
| `reason` | Meaning |
|----------|---------|
| `committed` | The user committed to a quote. The auction is over, so check `won`. This can arrive well before `expires_at`; there is no minimum auction duration. |
...
Sent when a user opens an [early cashout auction](/guides/cashout-auctions) on a position, and once for each active auction replayed during the initial snapshot. These arrive on the same stream and are **broadcast to every market maker**, including positions you do not back. Bid with [`PUT /v1/mm/cashout-requests/{id}/quote`](/api-reference/quote-service-mm/cashout-quote). To pass, submit nothing; there is no decline endpoint. The auction lives about 10 seconds. See [Early cashout](/guides/cashout-auctions).
...
[Early cashout](/guides/cashout-auctions).

## Leg fields

Legs use the same field names on every event and both venues. What the fields hold is venue specific.
The examples above are Kalshi.

| Field | Kalshi | Polymarket |
| --- | --- | --- |
| `id` | Our leg id, a UUID. Stable for the life of the leg. Not a venue identifier. | Same. |
...
Legs are forwarded from the backend and can carry extra display fields beyond these. The set is
additive and differs by event, so parse leniently and ignore keys you do not use.

## Reconnection

Every SSE event includes an `id:` field. Persist this value. On reconnect, pass it as the `last_event_id` query parameter to replay any events you missed instead of receiving the full snapshot again. The stream retains a few minutes of events at peak traffic. If your id has aged out, you get the full snapshot again, starting with `snapshot_begin`, so treat every snapshot as authoritative.
...
- **Net stake.** `user_stake` is the net stake the MM underwrites: `user_stake = bet_amount - taker_fee`, where `taker_fee = floor(bet_amount_micro x taker_fee_bps / 10000)` is computed in integer microUSDC (1 USDC = 1e6 micro). A 25.00 bet at 100 bps gives a 0.25 fee and `user_stake` 24.75. Price `payout_odds` and size collateral against `user_stake`, **not** `bet_amount`. `bet_amount` stays the user's gross wager but is not the quoting base when the taker fee is on (`taker_fee_bps > 0`); when the fee is off, `user_stake == bet_amount`.
