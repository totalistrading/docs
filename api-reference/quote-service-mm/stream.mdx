...
`status` is always `active` here — you only ever receive requests you can still quote. Terminal transitions arrive as [`quote_request_expired`](#quote_request_expired), not as a `status` change on this event.
...
| `side` | `"yes"` or `"no"` | Same. |
| `resolution` | See below. Present on `cashout_request` legs only. | Same. |

Legs are sent verbatim from the backend, so a leg can also carry snapshot fields not listed above — event and market titles, per-side subtitles, image and venue URLs, strike geometry (`scalar_key`, `strike_type`, `floor_strike`, `cap_strike`), event exclusivity (`event_mutually_exclusive`), and on `cashout_request` the venue's raw `result`. These are additive and may grow without notice: parse leniently and ignore keys you don't use.
...
Present on `cashout_request` legs. Four values, same on both venues:
