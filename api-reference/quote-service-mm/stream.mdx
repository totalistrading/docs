...
<ResponseField name="legs" type="array" required>
  ...
  pricing an already-decided bet. `void` overrides the others: any void leg cancels the whole
  position at settlement and refunds the stake, so the position is worth roughly `user_stake` even
  when a sibling leg reads `lost`. With no void leg, one `lost` leg makes the bet worthless.
</ResponseField>
