...
The error codes and reasons a public integration branches on are listed on this page. Endpoint
pages add the reasons specific to them.
...
| `502` | Part of the work landed and the rest did not, such as `PARTIAL_VAULT_ONLY` on a withdrawal. | Read `error.details` before retrying. See [Funding](/guides/funding#when-it-fails). |
...
account's parlay exists. Quote request ids you do not own return `403`.
