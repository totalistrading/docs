...
| --- | --- | --- | --- |
| Credential does not start with `api_` | `403` | `401` | `wrong_api_key_type` |
| Well formed key, but invalid or revoked | `401` | `401` | |
| Valid key, missing the required scope | `403` | `403` | `insufficient_scope` |

A malformed key returns `403` on REST and `401` on SSE. The response body carries a hint about the
