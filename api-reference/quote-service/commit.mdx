...
When a commit is rejected, the `error.details.reason` field explains why. Every reason returns `409 Conflict` except `COMMIT_FAILED_RETRYABLE`, which returns `503`:
...
| `COMMIT_FAILED` | The commit could not be completed. No eligible market maker had sufficient collateral, for example. |
| `COMMIT_FAILED_RETRYABLE` | Transient error. Retry after a short delay. |
