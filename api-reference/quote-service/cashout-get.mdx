  ...
  <Expandable title="data">
    <ResponseField name="cashout_request" type="object">
      ...
      <Expandable title="cashout_request">
        ...
        <ResponseField name="legs" type="array">The position's legs (ticker, side, venue, and resolution where known). On Polymarket legs `market_ticker` carries the `condition_id` and `event_ticker` the Gamma event slug, which falls back to the market slug then the `condition_id`. See [Leg fields](/api-reference/quote-service-mm/stream#leg-fields).</ResponseField>
      </Expandable>
    </ResponseField>
