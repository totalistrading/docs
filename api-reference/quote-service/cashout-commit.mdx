...
| 503 | `SERVICE_UNAVAILABLE` | **Indeterminate** — the buyback may have landed. `details.indeterminate` is `true` and `details.reason` is e.g. `lane_unreachable` / `buyback_in_flight`; the request stays `committing` for the reconciler. Do not retry blindly. |
