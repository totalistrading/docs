...
The Totalis API is REST over HTTPS with JSON bodies, plus Server-Sent Events (SSE) for live quote pricing and a [WebSocket](/guides/websocket) for post-trade events. The wire format is `snake_case`. Not sure which channel fits your integration? Start with [Real-time & data channels](/guides/realtime-and-data).
