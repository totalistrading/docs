...
description: 'List your active market-maker positions, each with all its legs. Offset paginated, with an optional exposure filter.'
---

List your active positions as the market maker, newest first, each with all its legs. A position here is a trade where a user committed one of your quotes and you confirmed it.

Use it to render your live book, reconcile exposure, or resolve an `rfq_id` you already hold (for example from [Get vault](/api-reference/funds/get-vault)) to its legs. The bettor-scoped RFQ reads are keyed to the user who placed the bet and return `404` to you; this is the maker-side counterpart.
  ...
  <Expandable title="data">
    <ResponseField name="positions" type="array">
      ...
      <Expandable title="position">
        ...
        <ResponseField name="rfq_id" type="string">The RFQ id. Pass it to [Get position](/api-reference/mm/get-position). Matches the `rfq_id` from [Get vault](/api-reference/funds/get-vault).</ResponseField>
        ...
        <ResponseField name="total_payout" type="number">Total pot paid to the winner if the bet resolves (`user_stake + mm_risk`).</ResponseField>
        ...
        <ResponseField name="legs" type="array">
          ...
          <Expandable title="leg">
            ...
            <ResponseField name="exclusivity_group" type="string">Mutual-exclusivity key: legs sharing a value bet into a one-of-N outcome set. `null` when the leg is not in a group. This is a per-leg snapshot. To pre-check whether two markets can combine in a combo, use the `exclusion_keys` arrays on [`GET /v1/markets/list`](/api-reference/markets/list-flat).</ResponseField>
