...
Your active positions as the market maker, meaning the trades where a user committed one of your quotes and you confirmed it, each expanded to its full parlay legs. Newest first.
  ...
  <Expandable title="data">
    <ResponseField name="positions" type="array">
      ...
      <Expandable title="position">
        ...
        <ResponseField name="position_id" type="string">On-chain position id, hex encoded (32 lowercase chars, no `0x`). The join key for early cashout: match it against the `position_id` on a broadcast [`cashout_request`](/api-reference/quote-service-mm/stream#cashout_request) to tell whether you back that position.</ResponseField>
        ...
        <ResponseField name="legs" type="array">
          ...
          <Expandable title="leg">
            ...
            <ResponseField name="exclusivity_group" type="string">Mutual-exclusivity grouping key: legs sharing a value bet into a one-of-N outcome set. `null` when the leg is not part of a group. This is a per-leg snapshot; to pre-check whether two markets can combine in a parlay, use the `exclusion_keys` arrays on [`GET /v1/markets/list`](/api-reference/markets/list-flat) instead.</ResponseField>
