...
When a user commits your quote, you receive a `quote:accepted` WebSocket event. Confirm before its `confirmation_deadline`. The window is 5 seconds from acceptance by default, but it is configurable server-side, so honor the timestamp rather than a fixed duration.
