---
title: 'Confirm quote'
api: 'POST /v1/mm/quotes/{quoteId}/confirm'
...
You receive a `quote:accepted` WebSocket event when a user commits your quote, and must confirm before the `confirmation_deadline` in that event. The window is **5 seconds from acceptance by default**, but it is configurable server-side, so always honor the `confirmation_deadline` timestamp rather than assuming a fixed duration. You may also receive `mm_quote:accepted` on the same `mm:quotes:{mm_id}` channel, with richer quote economics and `exposure_legs`, for local risk reservation or analytics. Both events carry the same `quote_id` (either is valid for confirmation), and delivery order between them isn't guaranteed. If you miss the deadline, the trade unwinds: the RFQ and your accepted quote both become `expired`, with reason `mm_confirmation_timeout`.
