...
description: 'Totalis on-chain payment architecture — Solana vault PDAs, portfolio margining via ILP, Privy TEE wallet signing, and gas-sponsored transactions'
---
...
Each participant has exactly one vault, derived from their wallet address:
...
- **User wins** -> `settle_position(user_won: true)`: MM vault transfers `mm_risk` minus fee to user vault. Status: `SettledWin`
- **MM wins** -> `settle_position(user_won: false)`: User vault transfers `user_stake` minus fee to MM vault. Status: `SettledLoss`
- **Invalid leg** -> `cancel_position()`: Both vaults' collateral is unlocked, no fund transfer. Status: `Cancelled`
...
Cancellation unlocks collateral in both vaults — no fund transfers occur:

- User's `locked_collateral` decreases by `user_stake`
- MM's `locked_collateral` decreases by `mm_collateral_locked`
...
|---------|---------------|-------------|
| **vault-job-service** | 5s | Processes pending positions: resolves Privy wallets, calculates ILP exposure via mm-exposure-api, executes `create_position` |
| **vault-settlement-service** | 30s | Polls Kalshi API for market results. Settles or cancels active positions based on outcomes. |

### vault-job-service Flow

1. Atomically claim one pending position using a transaction-based lock
...
6. `close_position` is executed in the same TX as `settle_position`/`cancel_position`
