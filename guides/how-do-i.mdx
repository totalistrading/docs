...
## Find markets that can go in the same combo

Every market carries `exclusion_keys`. Two markets can share a combo only if their arrays do not
intersect. Check that yourself before you create the request.
See [Limits and fees](/guides/limits-and-fees#bet-construction).

## Place a bet

Three calls: create the quote request with your legs and a bet amount, watch the quote stream,
then commit the quote you want. Funds move only once the maker confirms.
See [How bets work](/guides/lifecycle#place-a-bet).
...
the app. Placing a bet moves the stake into the vault by itself, and you never need SOL.
See [Funding](/guides/funding#send-usdc-to-your-wallet).

## Enable trading

Once, in the app, when you first place a bet. It delegates signing to Totalis inside a secure
