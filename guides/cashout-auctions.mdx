...
icon: 'gavel'
description: 'Bid on early exits from bets as a market maker: one broadcast auction, one all-in price, two outcomes notified differently.'
---

Bid on the auction Totalis opens when a user leaves a bet early. The bettor's side is on
[Early cashout](/guides/early-cashout).

## At a glance

- A user can leave a live bet early. Totalis opens an auction of about 10 seconds and broadcasts it to every maker on the stream you already consume.
- You bid one all-in price. The highest live bid wins, and ties go to the earliest bid.
- If the maker who backs the position wins, the position closes and the bet shows as bought back.
...
<Steps>
  ...
  <Step title="Price the exit">
    Re-value the bet at current odds, and read `legs[].resolution` carefully.
    ...
    With no void leg, one `lost` leg makes the bet worthless.
...
It can be below their stake. A losing bet exits for less than it cost, and the user never pays
...
A bought-back bet reports a realized cashout P&L, not a win or loss. The exit is a real
