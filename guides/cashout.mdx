...
flowchart LR
    ...
    active -- buyback window (~1s) lapses --> expired
...
<Steps>
  <Step title="Open the cashout request">
    ...
    - **`expires_at`** is the request's own TTL horizon. The buyback window is deliberately short, about a second, so you aren't left waiting on one maker. That's not the position's market end; that's `position_expires_at`.
...
| `expired` | The buyback window (about a second) elapsed with no commit. An unfilled buyback also opens a [position takeover](#position-takeover). |
| `cancelled` | The request was cancelled before commit. |

There's no client cancel action for a cashout. If you decide not to close the position, just don't commit: the request expires on its own within about a second. (`cancelled` is a possible terminal status, but it isn't something you trigger from the API.)
...
A decline isn't the end of the road for the position. A dead-ended buyback automatically escalates to a **position takeover**, covered next. Escalation is immediate when the maker declines, or within about half a second when the short buyback window is lapsing with no live quote.
...
A takeover is a separate request from the cashout, with its own auction and quote object. Unlike a cashout it's broadcast to every maker, so bids compete and the highest live bid wins, because that's the most you'd receive. The auction window is longer than the buyback, about 15 seconds.

### How a takeover starts

There are two ways one opens:

- **Automatically.** When a buyback dead-ends (the counterparty declined, or its short window lapsed with no live quote), Totalis opens a takeover for the position on your behalf.
...
- **Two short windows.** The buyback lasts about a second; the takeover auction that can follow runs about 15 seconds. Both are far shorter than the parlay's own windows.
