...
There's no client cancel action for a cashout. If you decide not to close the position, just don't commit: the request expires on its own once its ~90 second TTL elapses. (`cancelled` is a possible terminal status, but it isn't something you trigger from the API.)
...
A decline isn't the end of the road for the position. A dead-ended buyback, whether the maker declined or the request expired unfilled, automatically escalates to a **position takeover**, covered next.
...
A takeover is a separate request from the cashout, with its own auction and quote object. Unlike a cashout it's broadcast to every maker, so bids compete and the highest live bid wins, because that's the most you'd receive.

### How a takeover starts

There are two ways one opens:

- **Automatically.** When a buyback dead-ends (the counterparty declined, or the cashout request expired unfilled), Totalis opens a takeover for the position on your behalf.
