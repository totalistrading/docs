...
| `buyback_position` | Early cashout: MM unwinds an `Active` position for `amount`, releasing the user's collateral, realizing the taker fee, and carving the profit fee (`amount × fee_bps`) from the receiving side. Recomputes MM collateral (release-only) and closes + tombstones the position in-place — no separate `close_position` | `fee_payer` + `user` + `mm` + `settlement_authority` |
