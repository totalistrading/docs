...
| `buyback_position` | Fee Payer | Fee Payer + User + MM (via Privy TEE) + Settlement Authority |
...
| `buyback_position` | Early cashout: MM unwinds an `Active` position for `amount`, releasing the user's collateral, realizing the taker fee, and carving the profit fee (`amount × fee_bps / 10_000`) from the receiving side. Recomputes MM collateral (release-only) and closes + tombstones the position in-place — no separate `close_position` | `fee_payer` + `user` + `mm` + `settlement_authority` |
...
buyback_position()  → (closed + tombstoned, from Active)
```
...
Early cashout (`buyback_position`) closes an `Active` position in-place — there is no distinct on-chain status variant for it; the position account is tombstoned and the RFQ is recorded as `bought_back` off-chain. See [RFQ Lifecycle → Early Cashout](/guides/lifecycle#early-cashout).
