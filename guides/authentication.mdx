...
- **API key** — for programmatic clients (bots, scripts, server-side integrations). Sent as the `X-API-Key` header on REST and SSE; on the WebSocket, send the same key in the auth message instead.
...
Keys have the form `api_live_...` (or `api_test_...` for test-environment keys), matching `api_(live|test)_[A-Za-z0-9_-]{28,36}`. A credential that doesn't start with `api_` is rejected with `403` (`wrong_api_key_type`) and a hint about the expected format; a well-formed but invalid or revoked key gets `401`.
...
`{prefix}` is the `key_prefix` value returned by both list and create: the short leading fragment of the key that listings expose in place of the full secret. The full key is returned exactly once, in `data.key` at creation.
