...
| `counterparty_mm_id` | The [exit auction object](/api-reference/quote-service/cashout-create) — the maker that **backs** the position, not a bidder. Informational: the auction is broadcast to everyone, and the server compares the winner against this id to decide whether the exit closes the position or transfers it. Compare it against the winning `best_quote.market_maker_id`; they are equal only in the closing case. (Was `target_mm_id` when a cash-out was offered to a single MM.) |
