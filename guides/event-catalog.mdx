...
| Exited early (cash-out auction) | not on this stream — see the separate [exit-auction stream](/api-reference/quote-service/cashout-stream), whose own terminal `status` event reports `"committed"` | `cashout_request` announces only the **opening** of the auction, broadcast to every market maker; there is no SSE event for its completion — see [Early exit auctions](/guides/early-exit-auctions) | `position:bought_back` when the winning bid closes the position (delivered to the seller and to the winning market maker). **Nothing is delivered when a bid instead acquires the position** — see [the transfer gap below](#the-transfer-notification-gap) | `position.bought_back` (`amount`, `mm_pays_user`, `tx_signature`) on a close; `position.transferred` is attributed to the seller only | RFQ `status: "bought_back"`, `cashout` object populated |
...
An early exit resolves one of two ways, and only one of them notifies you live.

**A closing exit is delivered over WebSocket.** `position:bought_back` reaches the seller and the winning market maker (on `mm:quotes:{mm_id}` and their user channel), carrying `rfq_id`, `position_pda`, `amount`, `mm_pays_user`, `tx_signature` and `quote_id`. The `position.bought_back` webhook carries the same moment for integrations that prefer webhooks.
...
`mm_id`, `market_maker_id`, and `counterparty_mm_id` all identify the same thing — the market maker quoting or backing a trade. Which spelling you see depends on the channel:
...
| `counterparty_mm_id` | The [exit auction object](/api-reference/quote-service/cashout-create) — the position's original counterparty. Informational only: the auction is broadcast to every market maker, and a winning bid from this one closes the position instead of transferring it. (Was `target_mm_id` when a cash-out was offered to a single MM.) |
