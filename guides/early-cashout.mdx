...
- A user can leave an `executed` parlay early. Totalis opens an auction of about 10 seconds and broadcasts it to every maker.
- You bid one all-in price in microUSDC. The highest live bid wins, and ties go to the earliest bid.
- If the backing maker wins, the position closes and the RFQ becomes `bought_back`.
- If any other maker wins, the position transfers to them and stays `executed`. The buyer gets no event today.
