...
<Steps>
  <Step title="User Creates RFQ">
    The user submits a parlay bet request with 2–5 legs. Each leg references a market on its source venue (`kalshi` or `polymarket`) and a side (`yes`/`no`).
  ...
  <Step title="User Accepts Best Quote">
    ...
    Any other pending quotes on the same RFQ move to `rejected`. The response returns a `confirmation_deadline` — the timestamp by which the market maker must confirm.
