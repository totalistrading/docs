...
active → committing → committed     (success → the parlay becomes bought_back)
   ...
   └─ you cancel the request                          → cancelled
```
...
<Steps>
  ...
  <Step title="The maker prices it">
    The backing maker revalues the position at current odds and either streams a buyback quote or declines (if a leg is already decided). The quote carries the net you'd receive.
  </Step>
  <Step title="Commit">
    ```bash
