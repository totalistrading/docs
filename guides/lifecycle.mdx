...
<Steps>
  <Step title="User Builds a Live Quote Request">
    ...
    `market_ticker` is venue-polymorphic — see [Create RFQ](/api-reference/rfqs/create) for the full field reference:
    ...
    The system validates that each market exists in the cache, enforces leg-exclusion rules (see [Get Exclusion Groups](/api-reference/markets/exclusion-groups)), versions the request, and broadcasts it on `quote_requests:open` so market makers can quote against it.

    **Quote request status: `active`**
  </Step>

  <Step title="Market Makers Submit Draft Quotes">
    Market makers subscribe to the `quote_requests:open` WebSocket channel and submit draft quotes with their payout odds. [List Quote Requests](/api-reference/mm/list-quote-requests) is only a catchup/recovery endpoint for startup or reconnect gaps.
    ...
    POST /mm/quote-requests/<quote-request-id>/quotes
    {
      ...
      "leg_prices": [
        ...
        { "leg_id": "6e854379-57bb-487a-aec3-197fdd861fc8", "odds": 0.58 }
      ]
    }
    ```
...
| RFQ expiration | 30 s – 24 h (default 300 s); applies to real RFQs created at `commit-best` |
| Quote validity | 10 s – 1 h (default 60 s) |
| MM confirmation window | Server-controlled; returned on the `commit-best` response as `confirmation_deadline`. Missing it reverts the RFQ to `open` (quote → `withdrawn`) or, past `expires_at`, expires both. |
