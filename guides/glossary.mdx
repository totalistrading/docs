...
<img className="block dark:hidden rounded-2xl" src="/images/pictures/glossary-light.png" alt="Quote request before you commit, bet from commit, position from executed" />
<img className="hidden dark:block rounded-2xl" src="/images/pictures/glossary-dark.png" alt="Quote request before you commit, bet from commit, position from executed" />
...
| **Bet** | Any | The product name for the RFQ. Same object: say bet in a UI or conversation, use `rfq` in code. |
| **Single** | Any | A bet with one leg. |
| **Combo** | Any | A bet with two or more legs. |
...
| **Leg** | Any | One outcome inside a bet. Every leg must win for the bet to pay out. |
...
| `payout_odds` | | The multiplier applied to `user_stake`, never to `bet_amount`. Range on [Limits and fees](/guides/limits-and-fees#bet-construction). |
...
| `rfq_id` | The bet, after commit. The primary key for looking up trades. |
...
| `exclusion_keys` | An array on every market. Two markets can share a combo only if their arrays do not intersect. This is how correlated legs are blocked. |
...
| `outcome` | A settled bet's result, per leg and overall. |

## Custody terms

| Term | Definition |
| --- | --- |
| **Vault** | Your persistent on-chain Solana account, reused across every bet. Holds USDC and tracks gross, locked, and free balances. |
