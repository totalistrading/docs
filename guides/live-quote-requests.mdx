...
Live quote requests are the core trading mechanism on Totalis. You create a parlay with 2 to 5 market legs, market makers compete to price it in real time, and you lock in the best offer when you're ready.
...
| --- | --- |
| `QUOTE_CHANGED` | The request was updated since your last fetch. Refetch and try again. |
...
Keep quotes short lived (default 15s). When the request changes to a new version, your prior version quote is invalidated automatically — just reprice and `PUT` a new quote with the new `version` and `request_hash`. You don't need to withdraw the old one.
