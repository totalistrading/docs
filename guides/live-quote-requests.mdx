...
Keep quotes short-lived (default 15s). When the request changes to a new version, your prior-version quote is invalidated automatically — just re-price and `PUT` a new quote with the new `request_version` and `request_hash`. You don't need to withdraw the old one.
