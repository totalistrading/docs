...
Position events are delivered on the `rfq:{rfq_id}` channel (RFQ owner only). `position:created`, `position:failed`, and `position:error` are also sent to the MM's `mm:quotes:{mm_id}` channel. `position:settled` and `position:cancelled` are currently only sent to the RFQ owner.
