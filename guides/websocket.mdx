...
<Note>
  WebSocket events reach you only while you're connected — missed events are not redelivered. For durable, at-least-once delivery to a server (signed, retried, replayable), use [webhooks](/guides/webhooks); they carry most of the same lifecycle moments, plus one exit outcome WebSocket doesn't (early-cashout buyback — see the [event catalog](/guides/event-catalog#the-transfer-notification-gap)). See [Real time & data channels](/guides/realtime-and-data) for the full channel map.
</Note>
