...
| `position:bought_back` | A position was closed early by a winning cash-out bid. Delivered to the seller and to the winning market maker (on `mm:quotes:{mm_id}` and their user channel). `amount` is decimal USDC; `quote_id` is the position's original accepted quote, not the winning bid. Nothing is emitted when a bid *acquires* the position instead — see [the transfer gap](/guides/event-catalog#the-transfer-notification-gap). | `{ rfq_id, position_pda, amount, mm_pays_user, tx_signature, quote_id, timestamp }` |
