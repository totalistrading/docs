...
The channel key is your **`mm_id`** — confirmed to you in the [`connected`](/api-reference/quote-service-mm/stream#connected) event when you open the MM SSE stream. Do not assume it equals the `user_id` returned by `auth:success` on this WebSocket.
...
// From the `connected` event on the MM SSE stream
const MM_ID = '<your-mm-id>';
