...
|-------|-------------|------|
| `quote:accepted` | A user committed your quote. You must confirm it before `confirmation_deadline` (5 s from acceptance by default, but configurable server-side, so always use the timestamp). | `{ quote_id, rfq_id, confirmation_deadline }` |
