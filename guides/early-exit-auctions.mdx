...
<Steps>
  ...
  <Step title="Price the exit">
    Re-value the parlay at current odds, and read `legs[].resolution` carefully — the values are
    not interchangeable. A `lost` leg makes the parlay worthless. A `void` leg is different: that
    position is cancelled at settlement and the stake refunded, so it is worth about the stake, not
    zero. A near-decided leg is where mispricing hurts most. Then read
...
Your bid must also fit inside your locked collateral. `mm_risk` arrives as decimal USDC, so scale
it before comparing against a micro-USDC bid:

```
P <= S + round(mm_risk * 1e6) * (1 - f)        # S is user_stake * 1e6
```
