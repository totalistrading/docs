...
| [Webhooks](/guides/webhooks) | **Post-trade events, durable** — most of the same lifecycle moments as HMAC-signed POSTs with retries and replay, plus one moment WebSocket doesn't carry at all (a bid that **acquires** a position rather than closing it — see the [event catalog](/guides/event-catalog#the-transfer-notification-gap)) | Server systems of record that must never miss an event |
...
<Warning>
  The two channels are not full parity, but not over cash-out closes: `position:bought_back` **is** delivered over WebSocket (to the seller and the winning market maker) as well as by webhook. The gap is the other exit outcome — a winning bid that **acquires** a position emits nothing to the buyer on either channel, so an acquiring market maker learns of it only by reconciling against the [read endpoints](/guides/reading-data). See the [event catalog](/guides/event-catalog#the-transfer-notification-gap) for the full picture.
</Warning>
...
1. Stream quote requests and cashout requests from [`GET /v1/mm/quote-requests/stream`](/api-reference/quote-service-mm/stream).
