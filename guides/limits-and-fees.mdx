...
| --- | --- | --- | --- |
| Authenticated | `/v1/*` except `/v1/markets*` and the quote service routes, plus the flat roots below | your account | 300 / minute, 100 without a key |
...
The authenticated bucket also covers `/app/*`, `/rfqs*`, `/parlay-shares*`, and the flat
`/mm/quotes*` and `/mm/positions*` aliases. Every route in it draws down the same 300, including the
