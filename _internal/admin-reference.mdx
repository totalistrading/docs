...
<Warning>
  This page is internal and should NOT be included in the public Mintlify navigation (docs.json). It documents admin only endpoints and deprecated authentication methods.
</Warning>
...
For web applications using [Privy](https://privy.io) authentication. After a user logs in via Privy, the frontend receives a JWT token, used internally by the Totalis frontend — external API users should use API keys instead.
