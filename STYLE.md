...
- **Short sentences.** One idea each. Split sentences that run together. Use em dashes sparingly — at most one per
  sentence.
...
- **One term per concept.** A *parlay* is the product. An *RFQ* is the API object that represents
  one. A *quote request* is the pretrade price discovery object. A *position* is the on chain,
  funded parlay. Pick the right term for the layer and don't swap partway through the page.
- **Code works when you copy and paste it.** Show a real `curl` (or fenced HTTP) with the auth header, realistic
...
- **Guides** — task oriented walkthroughs: reading your data, webhooks, real time, market making.
...
Public guides are scoped to the principal; never document `X-Subaccount` or sub account variants in the
public tree.
