...
for (const c of confirmations) {
  const ok = stillWantThisRisk(c.quote); // your own check of prices and vault headroom
  const body = ok
    ? {
        decision: "CONFIRM",
...
- Sign the terms in the request, which are the terms you published. `toMessage` is the function from
  step 5.
