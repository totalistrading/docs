...
`stream.event` frames. With `rfq_details: true`, every RFQ event carries the full RFQ:
...
function buildQuote(rfq: any, payout: bigint, feeBps: number, takerFeeBps: number) {
  ...
  return { quote, message, digest: hashTypedData({ domain, types, primaryType: "Quote", message }) };
}

const { quote, message, digest } = buildQuote(rfq, 61_000_000n, 100, 20);
...
When a taker takes your quote, the venue opens a confirmation request and gives you 2 seconds. Poll
for requests continuously:

```ts
const { confirmations } = await get("/v1/maker/confirmations");
for (const c of confirmations) {
  const ok = stillWantThisRisk(c.quote); // re-check prices and headroom
  const body = ok
    ? {
        decision: "CONFIRM",
        maker_signature: await quoteSigner.signTypedData({
          domain, types, primaryType: "Quote", message: toMessage(c.quote),
        }),
      }
    : { decision: "DECLINE" };
...
- `toMessage` converts the API quote into the EIP-712 message exactly as in `buildQuote`. Sign the
  terms in the request, which are the terms you published.
- `quoteSigner` is a viem account for your quote signer key. The contract rejects any other signer.
- The `Idempotency-Key` must equal `attempt_id`.

| Result `state` | Meaning |
| --- | --- |
| `COMMITTED` | Your confirmation was accepted and the fill is queued on-chain. |
...
You can also publish a quote that is already signed with
[Submit quote](/hyperliquid/api-reference/rfqs-and-quotes/submit-quote): send the same `quote` and
`quote_digest` plus `maker_signature`. A taker can then take it without the confirmation round trip,
so you cannot decline it. A firm quote stays executable until its `expiry` even if you withdraw it
from the book, so keep expiries short.
...
on the same domain. When the owner takes your bid, confirm it exactly as in step 6, signing the
`SellBack` terms. Buying the ticket for your own book instead uses a `transfer` bid signed as
`CashoutTransfer`.
...
| Read | Scope | Returns |
| --- | --- | --- |
| [List maker positions](/hyperliquid/api-reference/maker-reads/list-maker-positions) | `vault:read` | Open and settled tickets against your maker. |
| [List maker activity](/hyperliquid/api-reference/maker-reads/list-maker-activity) | `vault:read` | Fills, settlements, and vault movements. |
| [Get maker performance](/hyperliquid/api-reference/maker-reads/get-maker-performance) | `vault:read` | P&L and volume over a period. |
| [List maker quotes](/hyperliquid/api-reference/realtime/list-maker-quotes) | `quote:read` | Your live quotes. |

The position and activity reads require the `X-Totalis-Account-Contract: 1` header plus the
`chain_id` and `vault` query parameters from [Get vault release](/hyperliquid/api-reference/vault/get-vault-release).

## Withdrawing capital and rotating the signer

Maker withdrawals and quote signer changes are made by the controller wallet directly on the vault
contract, not through the API:
