...
// Sends a request, resending the same bytes after a network failure or an AFTER_BACKOFF or
// BACKOFF error.
async function send({ path, idempotencyKey, body }: Request, attempts = 3): Promise<any> {
  for (let attempt = 1; ; attempt++) {
    ...
    try {
      ...
      text = await res.text(); // a dropped connection fails here too, and is retried
    } catch (networkError) {
    ...
    const retry = data?.error?.retry;
    if (attempt >= attempts || (retry !== "AFTER_BACKOFF" && retry !== "BACKOFF")) throw data;
    const retryAfter = Number(res.headers.get("Retry-After"));
    await sleep((retryAfter >= 1 ? retryAfter : attempt) * 1000);
  }
}
...
`send` retries a lost or throttled request with the **same** key and bytes, so you get the original
result instead of a second bet. To retry later still, build the request with `request` yourself,
store it, and pass the same object to `send` each time. See [Idempotency](/hyperliquid/conventions#idempotency).
