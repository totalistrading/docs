...
| USDC in your Totalis balance | Deposit in the [Totalis app](https://app.totalis.trade). The stake is taken from your vault balance. |

## 1. Set up a client

Every command is a `POST` with the same envelope and a fresh `Idempotency-Key`. Keep both in one
helper:
...
async function command(path: string, command: object, idempotencyKey = uuidv7()) {
...
If a request times out, resend it with the **same** `Idempotency-Key` and body. You get the original
result instead of a second bet. See [Idempotency](/hyperliquid/conventions#idempotency).
...
const balances = await get("/v1/me/product-balances");
```

`vault.chain_id` and `vault.vault_address` go into every signature's EIP-712 domain. Read them from
the API rather than hard-coding them. In `balances`, the `TOTALIS` product's `VAULT_FREE` component
is what a stake draws from.
...
async function bestQuote(rfqId: string) {
  ...
  return quotes
    ...
    .sort((a: any, b: any) => (BigInt(b.quote.payout) > BigInt(a.quote.payout) ? 1 : -1))[0];
}
...
// 2. Read the bids and take the highest.
const { quotes: bids } = await get(`/v1/rfqs/${cash.rfq_id}/cashout-book`);
const bid = bids.filter((b: any) => b.sell_back)
  .sort((a: any, b: any) => (BigInt(b.sell_back.price) > BigInt(a.sell_back.price) ? 1 : -1))[0];
