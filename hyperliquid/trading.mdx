...
// Sends a request, resending the same bytes after a network failure or an AFTER_BACKOFF error.
async function send({ path, idempotencyKey, body }: Request, attempts = 3): Promise<any> {
  for (let attempt = 1; ; attempt++) {
    ...
    if (attempt >= attempts || (retry !== "AFTER_BACKOFF" && retry !== "BACKOFF")) throw error;
